...
/**
 * LISTAR USUARIOS
 * GET /api/users?search=&role_id=&role=&center_id=&active=1&page=1&pageSize=20
 * - search: busca en rut, nombre, email o username (ILIKE)
 */
router.get("/", async (req, res) => {
  ...
  if (search) {
    ...
    where.push(
      `(u.rut ILIKE $${n + 1} OR u.nombre ILIKE $${n + 2} OR u.email ILIKE $${n + 3} OR u.username ILIKE $${n + 4})`
    );
  }

  if (role_id) {
    p.push(Number(role_id));
    where.push(`u.role_id = $${p.length}`);
  }

  if (role) {
    p.push(role);
    where.push(`r.role_name = $${p.length}`);
  }

  if (center_id) {
    p.push(center_id);
    where.push(`u.center_id = $${p.length}`);
  }

  if (active === "1" || active === "0") {
    p.push(active === "1");
    where.push(`u.is_active = $${p.length}`);
  }
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!rs.rowCount) return res.status(404).json({ error: "Usuario no encontrado" });
    res.json(rs.rows[0]);
  } catch (err) {
...
router.post("/", async (req, res) => {
  try {
    ...
    if (e?.code === "23505") {
      // unique violation (rut/email/username)
      return res
        .status(409)
        .json({ error: "RUT, email o username ya existe" });
    }
...
/**
 * ACTUALIZAR USUARIO (no password aquí)
...
router.patch("/:id/password", async (req, res) => {
  ...
  if (!password || password.length < 8) {
    return res.status(400).json({ error: "Password requerido (mínimo 8 caracteres)" });
  }
  try {
    const hash = await bcrypt.hash(password, 10);
    const rs = await pool.query(
