...
function normalizeRole(input: string | null | undefined): AssignmentRole {
  ...
  if (
    ...
    s.includes('comunidad') || s.includes('community') ||
    s === 'contacto'
...
const addAssignmentHandler: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    // ✅ Validación previa: usuario existe y está activo
    ...
    if (userRs.rows[0].is_active === false) {
      await client.query('ROLLBACK');
      res.status(400).json({ error: 'Usuario inactivo.' });
      return;
    }

    // ✅ Validación previa: centro existe
    const centerRs = await client.query(
      'SELECT 1 FROM centers WHERE center_id = $1',
      [center_id]
    );
    if (centerRs.rowCount === 0) {
      await client.query('ROLLBACK');
      res.status(404).json({ error: 'Centro no existe.' });
      return;
    }
    ...
    if (normRole === 'contacto ciudadano') {
      ...
      const col2 = centersPointerColumn(normRole); // 'comunity_charge_id'
      await client.query(
        `UPDATE centers
            SET ${col2} = NULL, updated_at = NOW()
          WHERE ${col2} = $1
            AND center_id <> $2`,
        [user_id, center_id]
      );
    ...
    await client.query('ROLLBACK'); // 🔧 CHANGE: antes hacía pool.query('ROLLBACK'); ahora usamos el MISMO client
    // Errores típicos
    if (e?.code === '23503') {
